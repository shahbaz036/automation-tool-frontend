...
const Login = ({ onLogin, onNavigate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('https://automation-tool-backend-1.onrender.com/api/users/login', {
        method: 'POST',
...
const Signup = ({ onSignup, onNavigate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch('https://automation-tool-backend-1.onrender.com/api/users/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password }),
      });
